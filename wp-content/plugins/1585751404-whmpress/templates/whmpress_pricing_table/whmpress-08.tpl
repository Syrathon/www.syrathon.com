...
{$primary_color='#2EDB7C'}
{/if}

{if $secondary_color eq '' || $secondary_color eq '#000000'}
{$secondary_color='#dcf9e9'}
{/if}
<style>
	#{$random_id} .pricing_table_icon{
	color: {$primary_color};
	}
	#{$random_id}:before {
...
<div class="whmpress whmpress_pricing_table eight {$featured}" id="{$random_id}">
	...
	<div class="pricing_table_heading">
		<div class="holder">
			<h2>{$product_name} - <span class="currency">{$prefix}</span><span class="amount">{$amount}</span>{if $fraction ne ""}<span
